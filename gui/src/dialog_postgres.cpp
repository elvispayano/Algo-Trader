...
DialogPostgres::DialogPostgres( QWidget* parent )
  ...
  // Button actions
  QObject::connect(
      ui->pushConnect, SIGNAL( released() ), this, SLOT( connectReleased() ) );
  QObject::connect(
      ui->pushCancel, SIGNAL( released() ), this, SLOT( cancelReleased() ) );
  QObject::connect(
      ui->textHost, SIGNAL( textChanged() ), this, SLOT( updateHost() ) );
  QObject::connect(
      ui->textPort, SIGNAL( textChanged() ), this, SLOT( updatePort() ) );
  QObject::connect(
      ui->textOptions, SIGNAL( textChanged() ), this, SLOT( updateOptions() ) );
  QObject::connect(
      ui->textTTY, SIGNAL( textChanged() ), this, SLOT( updateTTY() ) );
  QObject::connect( ui->textDatabase,
                    SIGNAL( textChanged() ),
                    this,
                    SLOT( updateDatabase() ) );
  QObject::connect( ui->textUsername,
                    SIGNAL( textChanged() ),
                    this,
                    SLOT( updateUsername() ) );
  QObject::connect( ui->textPassword,
                    SIGNAL( textChanged() ),
                    this,
                    SLOT( updatePassword() ) );
...
void DialogPostgres::connectReleased( void ) {
  ...
  if ( host.empty() || port.empty() || db.empty() || user.empty() ||
       password.empty() ) {
  ...
  database = new Postgres( ( char* )host.c_str(),
                           ( char* )port.c_str(),
                           ( char* )opt.c_str(),
                           ( char* )db.c_str(),
                           ( char* )db.c_str(),
...
void DialogPostgres::highlight( QPlainTextEdit* element,
