...
DialogNetworkCreate::DialogNetworkCreate( QWidget* parent )
  ...
  QObject::connect(
      ui->pushAdd, SIGNAL( released() ), this, SLOT( onAddReleased() ) );
  QObject::connect(
      ui->pushRemove, SIGNAL( released() ), this, SLOT( onRemoveReleased() ) );

  // Layer order modification
  QObject::connect(
      ui->pushUp, SIGNAL( released() ), this, SLOT( onUpReleased() ) );
  QObject::connect(
      ui->pushDown, SIGNAL( released() ), this, SLOT( onDownReleased() ) );

  // Accept/Reject
  QObject::connect(
      ui->buttonBox, SIGNAL( accepted() ), this, SLOT( networkAccepted() ) );
  QObject::connect(
      ui->buttonBox, SIGNAL( rejected() ), this, SLOT( networkRejected() ) );
...
void DialogNetworkCreate::swapRow( int x, int y ) {
  for ( int i = 0; i < ui->tableNetwork->columnCount(); ++i ) {
    if ( i == 2 ) {
      QSpinBox* box1 =
          static_cast< QSpinBox* >( ui->tableNetwork->cellWidget( x, i ) );
      QSpinBox* box2 =
          static_cast< QSpinBox* >( ui->tableNetwork->cellWidget( y, i ) );
      ...
      QComboBox* box1 =
          static_cast< QComboBox* >( ui->tableNetwork->cellWidget( x, i ) );
      QComboBox* box2 =
          static_cast< QComboBox* >( ui->tableNetwork->cellWidget( y, i ) );
