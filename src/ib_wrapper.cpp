...
*/
void IBWrapper::orderMarket( std::string ticker,
                             std::string action,
                             double      quantity ) {
  clearContract();
  contractRequest.symbol = ticker;
...
*/
void IBWrapper::orderLimit( std::string ticker,
                            std::string action,
                            double      quantity,
                            double      price ) {
  clearContract();
...
*/
void IBWrapper::orderStop( std::string ticker,
                           std::string action,
                           double      quantity,
                           double      price ) {
  clearContract();
...
void IBWrapper::tickOptionComputation( TickerId tickerId,
                                       TickType tickType,
                                       double   impliedVol,
                                       double   delta,
                                       double   optPrice,
                                       ...
                                       double   theta,
                                       double   undPrice ) {
  printf( "TickOptionComputation. Ticker Id: %ld, Type: %d, ImpliedVolatility: "
          "%g, Delta: %g, OptionPrice: %g, pvDividend: %g, Gamma: %g, Vega: "
          "%g, Theta: %g, Underlying Price: %g\n",
          tickerId,
          ( int )tickType,
          impliedVol,
          delta,
...
//! [tickstring]
void IBWrapper::tickString( TickerId           tickerId,
                            TickType           tickType,
...
void IBWrapper::tickEFP( TickerId           tickerId,
                         TickType           tickType,
                         double             basisPoints,
  ...
  printf( "TickEFP. %ld, Type: %d, BasisPoints: %g, FormattedBasisPoints: %s, "
          "Total Dividends: %g, HoldDays: %d, Future Last Trade Date: %s, "
          "Dividend Impact: %g, Dividends To Last Trade Date: %g\n",
          tickerId,
          ( int )tickType,
          basisPoints,
...
//! [orderstatus]
void IBWrapper::orderStatus( OrderId            orderId,
...
//! [openorder]
void IBWrapper::openOrder( OrderId           orderId,
                           const Contract&   contract,
                           const Order&      order,
                           const OrderState& orderState ) {
  printf( "OpenOrder. PermId: %i, ClientId: %ld, OrderId: %ld, Account: %s, "
...
//! [updateaccountvalue]
void IBWrapper::updateAccountValue( const std::string& key,
...
//! [updateportfolio]
void IBWrapper::updatePortfolio( const Contract&    contract,
                                 double             position,
                                 double             marketPrice,
                                 double             marketValue,
                                 double             averageCost,
...
//! [contractdetails]
void IBWrapper::contractDetails( int                    reqId,
...
//! [bondcontractdetails]
void IBWrapper::bondContractDetails( int                    reqId,
                                     const ContractDetails& contractDetails ) {
...
void IBWrapper::printContractDetailsMsg(
    const ContractDetails& contractDetails ) {
...
void IBWrapper::printContractDetailsSecIdList(
...
void IBWrapper::printBondContractDetailsMsg(
    const ContractDetails& contractDetails ) {
...
//! [execdetails]
void IBWrapper::execDetails( int              reqId,
                             const Contract&  contract,
...
//! [updatemktdepth]
void IBWrapper::updateMktDepth( TickerId id,
                                int      position,
                                int      operation,
                                int      side,
                                double   price,
...
//! [updatemktdepthl2]
void IBWrapper::updateMktDepthL2( TickerId           id,
                                  int                position,
...
//! [updatenewsbulletin]
void IBWrapper::updateNewsBulletin( int                msgId,
                                    int                msgType,
                                    const std::string& newsMessage,
                                    const std::string& originExch ) {
  printf(
      "News Bulletins. %d - Type: %d, Message: %s, Exchange of Origin: %s\n",
      msgId,
      msgType,
      newsMessage.c_str(),
      originExch.c_str() );
...
//! [historicaldataend]
void IBWrapper::historicalDataEnd( int                reqId,
...
//! [scannerdata]
void IBWrapper::scannerData( int                    reqId,
                             int                    rank,
                             const ContractDetails& contractDetails,
...
//! [realtimebar]
void IBWrapper::realtimeBar( TickerId reqId,
                             long     time,
                             double   open,
                             double   high,
                             double   low,
...
void IBWrapper::deltaNeutralValidation(
...
//! [position]
void IBWrapper::position( const std::string& account,
                          const Contract&    contract,
                          double             position,
                          double             avgCost ) {
  printf( "Position. %s - Symbol: %s, SecType: %s, Currency: %s, Position: %g, "
          "Avg Cost: %g\n",
          account.c_str(),
          contract.symbol.c_str(),
          contract.secType.c_str(),
...
//! [accountsummary]
void IBWrapper::accountSummary( int                reqId,
...
void IBWrapper::verifyCompleted( bool               isSuccessful,
...
void IBWrapper::verifyAndAuthMessageAPI( const std::string& apiDatai,
...
void IBWrapper::verifyAndAuthCompleted( bool               isSuccessful,
                                        const std::string& errorText ) {
...
void IBWrapper::displayGroupList( int reqId, const std::string& groups ) {
  printf(
      "Display Group List. ReqId: %d, Groups: %s\n", reqId, groups.c_str() );
}
//! [displaygrouplist]

//! [displaygroupupdated]
void IBWrapper::displayGroupUpdated( int                reqId,
...
//! [positionmulti]
void IBWrapper::positionMulti( int                reqId,
                               const std::string& account,
  ...
  printf( "Position Multi. Request: %d, Account: %s, ModelCode: %s, Symbol: "
          "%s, SecType: %s, Currency: %s, Position: %g, Avg Cost: %g\n",
          reqId,
          account.c_str(),
          modelCode.c_str(),
          contract.symbol.c_str(),
...
//! [accountupdatemulti]
void IBWrapper::accountUpdateMulti( int                reqId,
                                    const std::string& account,
                                    const std::string& modelCode,
...
//! [softDollarTiers]
void IBWrapper::softDollarTiers( int                                  reqId,
...
//! [mktDepthExchanges]
void IBWrapper::mktDepthExchanges(
...
//! [tickNews]
void IBWrapper::tickNews( int                tickerId,
                          time_t             timeStamp,
...
//! [tickReqParams]
void IBWrapper::tickReqParams( int                tickerId,
                               double             minTick,
...
//! [newsArticle]
void IBWrapper::newsArticle( int                requestId,
                             int                articleType,
...
//! [historicalNews]
void IBWrapper::historicalNews( int                requestId,
...
//! [rerouteMktDataReq]
void IBWrapper::rerouteMktDataReq( int                reqId,
                                   int                conid,
                                   const std::string& exchange ) {
...
//! [rerouteMktDepthReq]
void IBWrapper::rerouteMktDepthReq( int                reqId,
                                    int                conid,
                                    const std::string& exchange ) {
...
//! [pnlsingle]
void IBWrapper::pnlSingle( int    reqId,
                           int    pos,
                           double dailyPnL,
                           double unrealizedPnL,
                           double realizedPnL,
...
//! [historicalticks]
void IBWrapper::historicalTicks( int                                  reqId,
...
void IBWrapper::historicalTicksBidAsk(
...
//! [historicaltickslast]
void IBWrapper::historicalTicksLast(
...
//! [tickbytickalllast]
void IBWrapper::tickByTickAllLast( int                   reqId,
                                   int                   tickType,
                                   time_t                time,
                                   double                price,
                                   int                   size,
...
//! [tickbytickbidask]
void IBWrapper::tickByTickBidAsk( int                     reqId,
                                  time_t                  time,
                                  double                  bidPrice,
                                  double                  askPrice,
                                  int                     bidSize,
  ...
  printf( "Tick-By-Tick. ReqId: %d, TickType: BidAsk, Time: %s, BidPrice: %g, "
          "AskPrice: %g, BidSize: %d, AskSize: %d, BidPastLow: %d, "
          "AskPastHigh: %d\n",
          reqId,
          ctime( &time ),
          bidPrice,
...
//! [orderbound]
void IBWrapper::orderBound( long long orderId,
                            int       apiClientId,
                            int       apiOrderId ) {
...
//! [completedorder]
void IBWrapper::completedOrder( const Contract&   contract,
                                const Order&      order,
                                const OrderState& orderState ) {
  printf( "CompletedOrder. PermId: %i, ParentPermId: %lld, Account: %s, "
          "Symbol: %s, SecType: %s, Exchange: %s:, Action: %s, OrderType: %s, "
          "TotalQty: %g, CashQty: %g, FilledQty: %g, "
